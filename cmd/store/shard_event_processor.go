...
func (s *shard) OnShardRemoveEvent(cluster *topology.Cluster, resource *pb.StoreResource, shardInfo *pb.ShardInfo) {
	...
	if shardInfo.IsPermanentDelete {
		...
		s.deleteInMemoryFollowProgress(resource.GetAdminAddress(), VastoShardId(shardInfo.ShardId))

		if VastoServerId(shardInfo.ServerId) != s.serverId {
			s.clearProgress(resource.GetAdminAddress(), VastoShardId(shardInfo.ShardId))
		}
	}
